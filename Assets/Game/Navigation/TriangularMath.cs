...
namespace ZE.MechBattle.Navigation
{
    public static class TriangularMath
    {
        ...
        [BurstCompile]
        public static float3 TriangularToWorld(float3 tri, float d)
        {
            var v = new float3(tri.x + 0.5f, tri.y + 0.5f, tri.z + 0.5f) * d;
        ...
        // generated by Google AI
        [BurstCompile]
        public static float3 TriangularToWorld(IntTriangularPos tri, float d)
        {
            ...
            var v = new float3(tri.DownLeft + 0.5f, tri.Up + 0.5f, tri.DownRight + 0.5f) * d;
        ...
        [BurstCompile]
        public static IntTriangularPos WorldToTrianglePos(float3 pos, float triangleEdge)
        {
            // x is min, y is max
            int2 DefineAxleBorders(float3 normal)
            {
                var projection = math.dot((double3)pos, (double3)normal);
                var v = projection / triangleEdge;
        ...
        // generated by Google AI
        [BurstCompile]
        public static float3 WorldToTriangular(float3 pos, double triangleEdge)
        {
            var p = (double3)pos;

            var vX = math.dot(p, (double3)DirX) / triangleEdge;
            var vY = math.dot(p, (double3)DirY) / triangleEdge;
            var vZ = math.dot(p, (double3)DirZ) / triangleEdge;
